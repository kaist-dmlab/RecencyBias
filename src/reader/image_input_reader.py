...
class ImageReader(object):
    def __init__(self, dataset_name, data_path, num_train_files, num_train_images, num_val_images, height, width, depth,
                 num_classes, resize_height=32, resize_width=32):
    ...
    def inputs(self, train=True, normalize=True):
        filenames = []
        ...
        if self.resize_height != self.height or self.resize_width != self.width:
            print("Resize:", (self.height, self.width), "->", (self.resize_height, self.resize_width))
            record.image = tf.image.resize_images(record.image, [self.resize_height,self.resize_width])
    ...
    def data_read(self, batch_size, train=True, normalize=True, shuffle=False):
        t_id, t_image, t_label = self.inputs(train=train, normalize=normalize)
        if train:
